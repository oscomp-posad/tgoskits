...
// YUYV (4:2:2 packed) -> RGB888, BT.601 full-range (NEON + scalar tail, byte-identical).
// `npix` is the pixel count (width*height) and must be even. Exposed so a consumer
// can run the CPU color-conversion straight into a destination of its choosing (e.g.
// an NPU zero-copy input buffer) as the fallback when the RGA path is unavailable.
