        ...
        || attr.type_ == hw::ARMV8_CORTEX_A76_TYPE
    {
        ...
        match args.type_ {
            ...
            PerfTypeId::PERF_TYPE_TRACEPOINT => {
                // A tracepoint id in the dynamic-kprobe-events space (created
                // via `perf probe` -> `kprobe_events`) resolves to a kprobe.
                // `perf record -e probe:func` reads that id and opens it here;
                        ...
                        crate::tracepoint::kprobe_events::resolve(raw_id as u32)
                {
                    let kp_args = PerfProbeArgs {
                        config: PerfProbeConfig::Raw(if is_ret {
                            kprobe::PROBE_CONFIG_RETURN
                        } else {
                    ...
                    Box::new(kprobe::perf_event_open_kprobe(
                        kp_args,
                        sample_period,
                        sample_type,
                    )?)
