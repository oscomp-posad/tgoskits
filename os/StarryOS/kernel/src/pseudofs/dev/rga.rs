...
impl RgaDevice {
    ...
    /// `Arc<DmaBufFile>` that must stay alive for the operation's duration. The backing is
    /// DMA-coherent memory, so no explicit cache sync is required.
    fn resolve_buf(
        &self,
        raw: u64,
        handle_flag: bool,
        ...
        if handle_flag {
            ...
            let obj = resolve_contiguous_dmabuf(raw as c_int)
                .ok_or(VfsError::BadFileDescriptor)?;
    ...
    fn execute_blit(&self, req: &librga_abi::RgaReq) -> VfsResult<usize> {
        ...
        // DMA coherency: the /dev/dma_heap backing is DMA-coherent memory
        // (ax_dma::alloc_coherent_pages_dma32), so the CPU and the RGA engine
        // observe the same DRAM without any cache maintenance. Hold the Arc clones
        // across the whole op so a concurrent RELEASE_BUFFER cannot free a buffer
        // mid-blit.
        ...
        for _ in 0..500 {
            match core.poll_status() {
                RgaStatus::Done => {
                    core.finish();
                    // Coherent dma-heap memory: the engine's output is visible to
                    // the CPU immediately, with no cache invalidation needed.
                    return Ok(0);
                }
                RgaStatus::Error => {
                    let d = core.diag();
