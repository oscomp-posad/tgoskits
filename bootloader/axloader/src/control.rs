...
use boot_proto::{SERIAL_BOOT_PREFIX, SERIAL_PROTOCOL_VERSION, SERIAL_READY_PREFIX};
...
pub struct BootOffer {
    ...
    pub udp_transfer_id: u32,
    pub udp_block_size: usize,
}
...
fn parse_boot_offer(input: &str) -> Result<BootOffer, ControlError> {
    ...
    Ok(BootOffer {
        ...
        udp_transfer_id: u32::try_from(
            json_u64_field(input, "udp_transfer_id")
                .ok_or(ControlError::MissingField("udp_transfer_id"))?,
        )
        .map_err(|_| ControlError::InvalidNumber("udp_transfer_id"))?,
        udp_block_size: usize::try_from(
            json_u64_field(input, "udp_block_size")
