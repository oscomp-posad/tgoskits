...
| 文件 | 说明 |
|------|------|
| [`技术报告（决赛）.pdf`](技术报告（决赛）.pdf) | 技术报告（77 页，ctexart） |
| [`进展汇报（决赛）.pdf`](进展汇报（决赛）.pdf) | 进展汇报幻灯片（34 页，ctexbeamer） |

LaTeX 源码见本目录 [`report/`](report/) 与 [`slides/`](slides/)，构建方式见下文「从源码构建」。初赛进展汇报见 [`slides/进展汇报（初赛）.pdf`](../../slides/进展汇报（初赛）.pdf)。
