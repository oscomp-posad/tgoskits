...
| 文件 | 说明 |
|------|------|
| `技术报告（决赛）.pdf` | 技术报告（77 页，ctexart） |
| `进展汇报（决赛）.pdf` | 进展汇报幻灯片（34 页，ctexbeamer） |

初赛材料见仓库 `slides/进展汇报（初赛）.pdf`。

## 队员分工

- **洪世金（Joseph Joshua Anggita，队长）**：内核性能与调度——perf 观测、调度器与大小核、频率与电压 DVFS、sysbench 基准与整机对比；报告撰写与整体协调。
- **王乙凡**：内存、正确性与加速器驱动——内存与 THP、页表与 TLB 正确性、RGA 与 JPU 驱动、原生显示栈。
- **徐子航**：仿真与板级使能——QEMU RKNPU 功能级模型、板级 I/O 与网络、UVC 相机、reboot 系统调用、机器人拾球应用。
...
figures/      out/ 渲染图（数据图由 src/*.py + data/*.json 生成；架构与场景图为矢量素材）
