...
报告本体由 \texttt{make report} 构建，该命令进入 \texttt{report/} 后执行 \texttt{tectonic main.tex --outdir build}。正文采用 ctexart 文档类，需要 CJK 字体。所有数字集中于 \texttt{data/numbers.tex} 这一份权威表，正文仅引用宏名，不写入字面量。

图表遵循同一条溯源链。每张图的脚本仅读取一份 \texttt{figures/data/FNN.json} 作为数据源。\texttt{render.py} 将这些 JSON 渲染为 \texttt{figures/out/FNN.png}，正文经 \verb|\figph| 引用，图未生成时宏自动回退为占位框。构建前经 \texttt{lint.sh} 校验，扫描禁用串、裸 IP 与对比句式，并统计 \verb|\abl| 标记的数量。
