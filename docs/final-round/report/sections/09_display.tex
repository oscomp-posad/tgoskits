...
默认启用 adopt 路径，复用 U-Boot 已建立的 \texttt{dclk} 与 PHY；冷启动路径作为可选特性按需启用，暂不纳入共享的板级配置。做出这一取舍是因为该 OrangePi 配置同时供 rknpu 与 tennis 板级测试使用，不应因显示驱动而受损。冷启动因此仅在有人值守的显示点亮会话中启用。两种内核均可干净链接：adopt 路径 16.26 s 构建、无回归，冷启动路径 31.28 s release 构建、已将 \texttt{rockchip-hdptx-phy} 编入。

\subsection{已验证的部分与板级待验}

真正在目标平台上运行过的仅有 Stage 0，即 Qt 在 QEMU 上的渲染。日志显示 linuxfb 平台插件加载成功，完整经历 \texttt{QT\_DEMO\_STARTED} 至 \texttt{QT\_DEMO\_PASSED}，客户机为 Alpine v3.23 配 \texttt{qt6-qtbase-6.10.3-r0}（与构建容器 ABI 对齐），时钟为 82~KB 的 aarch64 musl 动态 PIE，运行于原生 arm64 的 \texttt{qemu-system-aarch64} 11.0.2。为使 Qt 就位，离线安装了 109 个包共 276~MiB，并把 rootfs 由 2048~MiB 扩至 3072~MiB。Stage 1 至 4 为主机测试过的寄存器算术与干净的内核链接。
...
尚未验证的部分需如实说明。四个驱动阶段均未上板，PHY 的 193 个模拟微调值、全部 reset/GRF/MMIO 常量以及整套时序均为尽力对齐主线的最优推断，从未在 RK3588 上运行，上板判据落在那两次 HDPTX-GRF 锁定轮询与一次 VOP2 寄存器转储。板载 DTB 中 \texttt{route\_hdmi0} 标记为 disabled，冷启动据此采用完整的原生初始化路径，由驱动自行依序拉起整条流水线。当前作用域硬编码为 1080p60，未支持 DDC/EDID 与热插拔，留待上板后再行加固。所有约 24 至 25 个提交均在工作树分支本地，尚未推送、尚未开 PR。作为一项广度贡献，该显示栈的定位为主机寄存器级测试 39 项通过、板级 ROPLL 像素时钟锁定待验证，本章不对其作出超出这一状态的判断。
