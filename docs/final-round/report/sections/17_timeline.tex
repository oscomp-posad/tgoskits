...
一个月内落地并验证的工作包括硬件 PMU perf 从单核到多核再到大小核、QEMU RKNPU 功能模拟、axtask SMP 分布、CPU DVFS 与 PVTPLL 电压调节、占用感知的 CFS 对等调度器、THP 首触、CLONE\_VM 快路径、系统调用成本下降与 TTFI 领先。原生显示栈主机测试通过，板级点亮已在上板会话中通过（ROPLL 锁定）。另有三项未能生效，各有其因：工作窃取负载均衡器因抖动回退，early-MMU 启动重排因无收益回退，DDR/DMC 变频受固件所限，构成多线程侧余下的主要带宽差距。
