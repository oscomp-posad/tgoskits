...
\begin{frame}
  \navsegment{2}
  \frametitle{原生显示栈 VOP2 / HDMI-QP / HDPTX（上板点亮验证）}
  \begin{columns}[T]
    ...
    \begin{column}{0.51\textwidth}
      \begin{itemize}
        \item 从零实现 \#![no\_std] RK3588 显示栈，Qt6 时钟经 \texttt{/dev/fb0} 上屏；四 crate 合计 39 项主机测试通过
        \item \headline{上板点亮验证通过}（RK3588）：HDPTX ROPLL 锁定（GRF STATUS=0x0e），VP0 1080p60 光栅实时扫描，\texttt{/dev/fb0} 绘出彩条，Qt6 时钟原生运行
        \item HDPTX 193 条寄存器写逐字移植主线，判据为逐位一致与两次 GRF 锁定轮询
        \item 上板前对抗复核已修两个 HIGH（FB 缓存一致性、config-done 锁存）
      \end{itemize}
    \end{column}
  \end{columns}
  \advance{从零打通板端显示通路}{显示栈四 crate 主机 39 项 + 上板点亮（ROPLL 锁定、1080p60、\texttt{/dev/fb0}、Qt6 运行）}
\end{frame}
...
\begin{frame}
  ...
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{2.3cm}>{\raggedright\arraybackslash}p{11.7cm}@{}}
    ...
    \headline{王乙凡} & QEMU RKNPU 功能级模拟器：全流水线 + IOMMU + INT8/FP 执行（\#19，106 qtests），多核协同（\#26）；机器人拾球应用（与徐子航共同） \\[4pt]
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.47\textwidth}
      \begin{itemize}
        ...
        \item 冷启动路径的上板点亮会话即在该测试台完成，插座硬断电保证挂死可无人值守恢复
      \end{itemize}
    \end{column}
  \end{columns}
  \advance{验证挂死的闭环不能靠被测系统自恢复}{智能插座带外冷复位使闭环无人值守，覆盖会挂死内核的改动}
\end{frame}
...
\begin{frame}
  ...
  \frametitle{后续工作与可复现}
  \begin{itemize}
    \item \headline{后续方向}\ 调度全量均衡（迁移代价门限与迟滞）、DVFS Phase 2 电压闭环加温控、DDR SIP 换 rkbin BL31、THP 通用化（首触扩展为通用大页，补齐后台合并与 madvise）、NPU 三核上真机并上游化 QEMU 模型
    \item \headline{诚实口径}\ 全部性能数为 RK3588 整机三次重复中位数，对照同机同频 Linux；两侧运行同一份遵循 Linux ABI 的可执行文件
    \item \headline{可复现}\ 数字集中于 \texttt{data/numbers.tex} 宏，图经 \texttt{render.py} 由 JSON 溯源，\texttt{scripts/board/boardctl.py} 暖重启回路，\texttt{lint.sh} 校验字号、口径与调色板一致性
  \end{itemize}
  \advance{各章给出对等与少数领先}{后续方向明确，全链可复现}
\end{frame}
