...
\begin{frame}
  ...
  \centering
  \includegraphics[width=\linewidth,height=0.72\textheight,keepaspectratio]{Fkmap.png}
\end{frame}

% ============================== 19a 感知加速器驱动 + 零拷贝 ==============================
\begin{frame}
  \navsegment{1}
  \frametitle{感知流水线：三块自研加速器驱动 + dma-buf 零拷贝}
  \begin{columns}[T]
    ...
    \begin{column}{0.45\textwidth}
      \begin{itemize}
        \item \headline{JPU}\ 硬件 JPEG 解码：新增 \texttt{/dev/mpp\_service}，对齐瑞芯微 MPP;单帧 1\,ms、与 Linux 逐字节一致（\#1456）
        \item \headline{RGA}\ 缩放与色彩转换：新增 \texttt{/dev/rga} RGA2 驱动，对齐 librga，较 CPU 快 7.4 倍（\#1388）
        \item \headline{NPU}\ 连续推理：修正 GEM 缓冲缺页与引用计数，逐帧稳定、首帧与 Linux 一致（\#1364）
        \item \headline{零拷贝}\ 三块加速器经 dma-buf 共享物理内存，整条流水线全程零拷贝
      \end{itemize}
    \end{column}
  \end{columns}
  \advance{感知三级加速全部本队自研}{JPU 硬解 + RGA 预处理 + NPU 连续推理，经 dma-buf 零拷贝串联，均已合入上游}
\end{frame}
...
\begin{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \includegraphics[width=\linewidth]{Finfer.png}
    \end{column}
    \begin{column}{0.45\textwidth}
      \begin{itemize}
...
\begin{frame}
  ...
  \centering
  \includegraphics[width=\linewidth,height=0.72\textheight,keepaspectratio]{F01_deck.png}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.43\textwidth}
      \begin{itemize}
        ...
        \item getpid 降至 \nGetpid\,ns，相对 Linux 由 7 倍收窄至 \nGetpidRatio 倍，stime 秒占比 0.71 对齐 Linux 0.72（残余为 \texttt{uctx.run} 陷入的架构地板）
      \end{itemize}
    \end{column}
  \end{columns}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.45\textwidth}
      \begin{itemize}
        ...
        \item \vwarn{DDR 变频}驱动已实现，因主线 TF-A 缺 SIP DRAM 的 EL3 处理器而停在探测态（外因）
      \end{itemize}
    \end{column}
  \end{columns}
...
\begin{frame}
  \navsegment{2}
  \frametitle{perf 真机实测：硬件 PMU stat / record / report}
  \centering
  \includegraphics[width=\linewidth,height=0.74\textheight,keepaspectratio]{Fperf_real.png}
\end{frame}

% ============================== perf 火焰图 ==============================
\begin{frame}
  \navsegment{2}
  \frametitle{perf 火焰图：网球推理的 CPU 调用栈}
  \centering
  \vfill
  \includegraphics[width=\linewidth]{Fperf_flame.png}\\[10pt]
  {\color{muted}\footnotesize \texttt{perf record -g} 采样 6735 次，符号经 \texttt{/proc/kallsyms} 解析 —— 完整用户态调用栈 \texttt{\_start} → \texttt{main} → \texttt{inference\_yolov8} → \texttt{convert\_image} / \texttt{crop\_and\_scale}，右支为 libjpeg 的 NEON 解码 \texttt{jsimd\_idct\_islow\_neon}}
  \vfill
\end{frame}
...
\begin{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.32\textwidth}
      \centering
      \includegraphics[width=0.80\linewidth]{Fwayland.png}\\[5pt]
      \IfFileExists{../figures/out/Fdisplay_board.png}%
        {\includegraphics[width=\linewidth]{Fdisplay_board.png}\\[2pt]
         {\color{muted}\scriptsize 真机 HDMI 输出：实时仪表盘（2× 加速）}}%
        {}%
    \end{column}
...
% ============================== 真机跑通：PostgreSQL + Weston ==============================
\begin{frame}
  \navsegment{2}
  \frametitle{真机跑通：PostgreSQL 数据库与 Weston 图形栈}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      \includegraphics[width=\linewidth,height=0.64\textheight,keepaspectratio]{Fdemo_pg.png}
    \end{column}
    \begin{column}{0.42\textwidth}
      \centering
      \IfFileExists{../figures/out/Fweston_shot.png}%
        {\includegraphics[width=\linewidth,height=0.62\textheight,keepaspectratio]{Fweston_shot.png}}%
        {\fbox{\parbox[c][0.42\textheight][c]{0.9\linewidth}{\centering\color{muted}\footnotesize 此处放 Weston 桌面实拍截图\\[4pt]存为 \texttt{figures/out/Fweston\_shot.png}}}}\\[5pt]
      {\color{muted}\footnotesize Weston 合成器 + GTK4 / Xwayland，四架构验证通过}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \includegraphics[width=\linewidth]{Fdemo_npu.png}
    \end{column}
    \begin{column}{0.42\textwidth}
      \begin{itemize}
        ...
        \item 在该模型上逐层重塑网球检测模型：SiLU 改 ReLU 使 LUT 任务 107$\to$3、\texttt{rknn\_run} 减半，叠加类别头收窄与头框合并
      \end{itemize}
    \end{column}
  \end{columns}
...
\begin{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.34\textwidth}
      \gfxt[0.80\linewidth]{F20.png}\\[3pt]
      \gfxt[0.62\linewidth]{F21.png}
    \end{column}
    \begin{column}{0.63\textwidth}
      \begin{itemize}
        ...
        \item 慢速轮速遥测移出控制回路，由后台线程异步更新位姿快照供每轮读取
      \end{itemize}
    \end{column}
  \end{columns}
