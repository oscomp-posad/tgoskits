...
\begin{frame}[plain]
  ...
  {\color{ink}\footnotesize 成果全部回馈上游 rcore-os/tgoskits（累计 \nPRMerged{} 个 PR 已合并）。}\\[16pt]
...
\begin{frame}
  ...
  \end{columns}
  \vspace{1.5mm}
  {\color{muted}\footnotesize 工作量以回馈上游 rcore-os/tgoskits 计量（自初赛至今累计；决赛期新增 \nPRFinalsMerged{} 已合并 + \nPRInflight{} 在审）：}
...
\begin{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.47\textwidth}
      ...
      \begin{itemize}
        \item 内核：rcore-os/tgoskits 的 dev 分支 @73409e079（StarryOS + ArceOS 组件）
        \item 机器人应用：移植自 pengzechen/aka-rk3588
        \item 模型转换：airockchip/rknn-toolkit2
...
\begin{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.55\textwidth}
      \begin{itemize}
        \item 人掌架构与验收，AI 在真机与 QEMU 闭环内迭代实现，运行至机器可判定的判定门：一次运行产出一个可比对数值
        \item 四类判定门：sysbench 对等回归、schbench 延迟阈值、多智能体对抗评审、s\_frac 计时 oracle；触及 SMP 同步的改动每次配 7 到 26 名评审
        \item 带外电源自治测试台：计量式智能插座（miIO 本地）分级恢复 L0 暖重启、L1 硬断电冷启、L2 U-Boot 金身、L3 人工，令会挂死内核的改动也可无人值守
...
\begin{frame}
  ...
  \frametitle{小结、分工与材料}
  \begin{itemize}
    \item \headline{贡献小结}\ 机器人推理追平 Linux；整 OS 对 Linux 逐轴对等，THP / TTFI / A55 / schbench 领先；QEMU NPU 设备模型与 AI 开发范式；全部成果回馈上游 rcore-os/tgoskits，累计 \nPRMerged{} 个 PR 已合并、\nPRInflight{} 个在审
    ...
    \item \headline{材料与复现}\ 同一份 ELF 两侧对照、三次重复中位数；代码见 rcore-os/tgoskits 与 processmission/qemu，报告与幻灯见 \texttt{docs/final-round/}
  \end{itemize}
  \vfill
...
\begin{frame}[plain]
  ...
  {\color{muted}\large 内核贡献全景（完整清单）· 目标完成表 · 基准与口径 · 增量 PR 一览 · 与类似工作对比 · 机器人状态机 · 视觉推理链路 · AI 使用说明}
  \vfill
\end{frame}
...
\begin{frame}
  \navsegment{0}
  \frametitle{内核贡献全景（完整清单）}
  \centering
  \includegraphics[height=0.84\textheight,keepaspectratio]{Fkmapfull.png}
\end{frame}
