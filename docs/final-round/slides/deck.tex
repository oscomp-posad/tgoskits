  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.45\textwidth}
      \begin{itemize}
        ...
        \item \vwarn{DDR 变频}走 Rockchip SIP DRAM 接口，主线 TF-A 缺该 EL3 处理器，驱动侧已就位，停在探测态，受主线固件外因所限
      \end{itemize}
    \end{column}
  \end{columns}
  \advance{CPU 固定于上电低频、落后约三十倍}{ondemand + PMIC + 环长调节逐核 \nPerCoreRatio× Linux；DDR 变频受主线固件外因所限}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.43\textwidth}
      \begin{itemize}
        ...
        \item 三项如实回退或受外因所限：工作窃取均衡器抖动回退、early-MMU 无收益回退、DDR 变频受主线固件外因所限
      \end{itemize}
    \end{column}
  \end{columns}
  \advance{整机对等分阶段推进}{七周里程碑，落地项均真板验证；三项如实回退或受外因所限}
\end{frame}
