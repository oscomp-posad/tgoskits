...
\begin{frame}
  ...
  \begin{itemize}\setlength{\itemsep}{9pt}
    \item \headline{一\ \ 背景与结果}\quad{\color{muted}命题、工作量总览、对 Linux 记分卡、系统分层、机器人回路}
    \item \headline{二\ \ 系统能力与性能}\quad{\color{muted}观测、调度、内存、启动、系统调用、频率、NPU 模型、显示与板级、正确性、回退}
    \item \headline{三\ \ 增量与展望}\quad{\color{muted}时间线、分工、主要贡献}
    \item \headline{四\ \ 方法与复现}\quad{\color{muted}AI 方法与自治测试台、后续工作、材料入口}
  \end{itemize}}
  \vfill
  {\color{muted}\footnotesize 顶部导航条标示当前所在段落；正文之后附有备用材料（附录）。}
\end{frame}
...
\begin{frame}
  \navsegment{1}
  \frametitle{一个月做了多少：工作量一览}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{itemize}
        \item \headline{32 项原子改动}\ 20 项已并入上游或 QEMU fork，7 项在审，5 项已在集成分支实现并板级验证
        \item \headline{6 个内核子系统}\ 观测 perf、调度与大小核、DVFS、内存与分页、页表正确性、启动 TTFI
        \item \headline{一套 QEMU RKNPU 设备模型}\ 把闭源 NPU 实现为功能级仿真，106 个 qtest
      \end{itemize}
    \end{column}
    \begin{column}{0.47\textwidth}
      \begin{itemize}
        \item \headline{约 8 个 RK3588 驱动}\ RGA、JPU、UART、USB-tty、PWM、UVC、网络、GEM
        \item \headline{原生显示栈 + 合成器}\ VOP2 / HDMI-QP / HDPTX 四 crate 上板点亮，Weston 跑通
        \item \headline{机器人拾球应用}\ 五阶段闭环在真机完整运行
        \item \headline{全部真机验证}\ perf 板级 39/0 与 56/0；性能数取三次重复中位数
      \end{itemize}
    \end{column}
  \end{columns}
  \advance{三名成员、约一个月}{32 项改动、6 个子系统、一套 QEMU NPU 模型、约 8 个驱动，全部 RK3588 真机验证}
\end{frame}

% ============================== 22 按轴诚实图例 ==============================
\begin{frame}
  \navsegment{1}
  \frametitle{对 Linux 逐轴记分卡（StarryOS 优化前 $\to$ 后）}
  \scriptsize
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.64\textwidth}
      \begin{itemize}
        ...
        \item \headline{Weston 合成器}\ 在 StarryOS 上跑通 Weston：补齐 \texttt{/dev/dri/card0} 原子 KMS、多路输入、\texttt{AF\_UNIX} 传 fd、\texttt{epoll} 就绪语义，四架构验证，图形栈 \#503--516 / \#1160 已合并
    ...
    \end{column}
  \end{columns}
  \advance{性能对比需先能驱动板子}{显示栈上板点亮、Weston 跑通、约 8 个板级驱动合入上游}
\end{frame}
...
\begin{frame}
  \navsegment{4}
  \frametitle{后续工作与材料入口}
  \begin{itemize}
    \item \headline{后续方向}\ 调度全量均衡（迁移代价门限与迟滞）、DVFS Phase 2 电压闭环加温控、DDR SIP 换 rkbin BL31、THP 通用化、NPU 三核上真机并上游化 QEMU 模型
    \item \headline{可复现}\ 全部性能数为 RK3588 整机三次重复中位数，对照同机同频 Linux，两侧运行同一份 ELF；\texttt{scripts/board/boardctl.py} 暖重启回路，数字集中于 \texttt{numbers.tex}
    \item \headline{材料}\ 代码见 rcore-os/tgoskits 与 processmission/qemu（\#19 / \#26），决赛报告与幻灯见 \texttt{docs/final-round/}
    \item \headline{致谢}\ 感谢 rcore-os 社区评审与竞赛组委会
  \end{itemize}
  \vfill
  \centering{\sffamily\color{teal}\large Team Posad\quad·\quad 清华大学}
  \advance{全部结论可追溯}{后续方向明确；代码、报告与板级回路全链可复现}
\end{frame}
