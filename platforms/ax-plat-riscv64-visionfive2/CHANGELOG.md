# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [Unreleased]

## [0.1.8](https://github.com/rcore-os/tgoskits/compare/ax-plat-riscv64-visionfive2-v0.1.7...ax-plat-riscv64-visionfive2-v0.1.8) - 2026-06-12

### Other

- updated the following local packages: rdrive, ax-plat, axklib

## [0.1.7](https://github.com/rcore-os/tgoskits/compare/ax-plat-riscv64-visionfive2-v0.1.6...ax-plat-riscv64-visionfive2-v0.1.7) - 2026-06-11

### Fixed

- fix typos in code and comments across the codebase ([#1206](https://github.com/rcore-os/tgoskits/pull/1206))

## [0.1.6](https://github.com/rcore-os/tgoskits/compare/ax-plat-riscv64-visionfive2-v0.1.5...ax-plat-riscv64-visionfive2-v0.1.6) - 2026-06-09

### Other

- updated the following local packages: ax-cpu, ax-kspin, rdrive, ax-plat, axklib

## [0.1.5](https://github.com/rcore-os/tgoskits/compare/ax-plat-riscv64-visionfive2-v0.1.4...ax-plat-riscv64-visionfive2-v0.1.5) - 2026-06-03

### Added

- *(irq)* introduce shared IRQ framework ([#1065](https://github.com/rcore-os/tgoskits/pull/1065))

### Fixed

- *(repo)* normalize allocator and RISC-V dependencies ([#1021](https://github.com/rcore-os/tgoskits/pull/1021))

### Other

- *(platform)* migrate riscv64 qemu to dynamic platform ([#1085](https://github.com/rcore-os/tgoskits/pull/1085))
- *(ax-alloc)* remove ax-allocator dependency, simplify to TLSF/buddy-slab backends ([#987](https://github.com/rcore-os/tgoskits/pull/987))

## [0.1.4](https://github.com/rcore-os/tgoskits/compare/axplat-riscv64-visionfive2-v0.1.3...axplat-riscv64-visionfive2-v0.1.4) - 2026-05-22

### Other

- updated the following local packages: ax-cpu

## [0.1.3](https://github.com/rcore-os/tgoskits/compare/axplat-riscv64-visionfive2-v0.1.2...axplat-riscv64-visionfive2-v0.1.3) - 2026-05-19

### Other

- Refactor Clippy integration and enhance package handling ([#738](https://github.com/rcore-os/tgoskits/pull/738))

## [0.1.2](https://github.com/rcore-os/tgoskits/compare/axplat-riscv64-visionfive2-v0.1.1...axplat-riscv64-visionfive2-v0.1.2) - 2026-05-15

### Other

- updated the following local packages: ax-kspin, ax-riscv-plic, ax-config-macros, ax-cpu, ax-plat
